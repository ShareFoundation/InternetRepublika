<?php
class mgThumbnailActions extends sfActions
{
  public function executeThumbnail()
  {
    ...
    sfContext::getInstance()->getConfiguration()->loadHelpers('Thumbnail');
    sfContext::getInstance()->getConfiguration()->loadHelpers('Tag');
    sfContext::getInstance()->getConfiguration()->loadHelpers('Asset');
    echo thumbnail_tag(sfConfig::get('sf_web_dir') . '/' . $imgname, $maxx, $maxy, array(), false, true);
    exit;
    ...
    $file_cache_dir = sfConfig::get('sf_cache_dir') . '/'.$namespace;
    $cache = new sfFileCache(array('cache_dir' => $file_cache_dir)); // Miskec fixed error 2008-11-07-15-32
    $cachename=md5($imgname.$maxx.$maxy.$ctime);
